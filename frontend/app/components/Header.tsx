...
export default function Header() {
  ...
  const getNavLabel = (link: (typeof NAV_LINKS)[number]) =>
    i18nNavLabel(link.key, lang);
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50">
              ...
              {showLangPicker && (
                <div className="absolute top-full right-0 mt-2 w-72 bg-obsidian/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                  <div className="px-4 pt-4 pb-2 border-b border-white/5">
                    <div className="text-[9px] font-mono text-terminal-green/40 tracking-[0.3em] uppercase mb-1">
                      CULTURAL UNIVERSE
                    </div>
                    <div className="text-xs text-white/30">
                      Select your content universe
                    </div>
                  </div>
                  <div
                  ...
                  >
                    {SUPPORTED_LANGUAGES.map((l) => (
                      <button
                        ...
                        className={`w-full px-4 py-2.5 flex items-center gap-3 text-left transition-all cursor-pointer ${
                          l.code === lang
            ...
            {isAuthenticated ? (
              (() => {
                const displayInitial =
                  user?.google?.name?.[0] || user?.email?.address?.[0] || "U";
                const avatarUrl =
                  (user?.google as Record<string, string> | undefined)
                    ?.picture || null;
                const userName =
                  user?.google?.name ||
                    ...
                    {showAvatarMenu && (
                      <div className="absolute top-full right-0 mt-2 w-56 bg-obsidian/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                        ...
                        <div className="px-4 py-3 border-b border-white/5">
                          <div className="text-sm font-medium text-ghost-white truncate">
                            {userName}
                          </div>
        ...
        {showLangPicker && (
          ...
          >
            <div className="text-[9px] font-mono text-terminal-green/40 tracking-[0.3em] uppercase mb-2 px-2">
              CULTURAL UNIVERSE
            </div>
            <div className="grid grid-cols-3 gap-1.5">
              {SUPPORTED_LANGUAGES.map((l) => (
                <button
                  ...
                  className={`px-2 py-2 rounded-lg text-center text-xs transition-all cursor-pointer ${
                    l.code === lang
