  ...
  try {
    ...
    financialTxs.forEach(tx => {
        ...
        if (dailyStats[dateStr]) {
            if (tx.type === "DEPOSIT") dailyStats[dateStr].depositsCC += tx.amount;
            if (tx.type === "CHAPTER_UNLOCK") dailyStats[dateStr].unlocksCC += Math.abs(tx.amount);
            if (tx.type === "TIP_SENT") dailyStats[dateStr].tipsCC += Math.abs(tx.amount);
        }
    });
